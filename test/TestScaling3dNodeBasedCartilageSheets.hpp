...
public:
	 ...
	 */
	void xTest3dNodeBasedCartilageSheet() throw (Exception) {
...
private:
	...
	void Setup3dNodeBasedCartilageSheet(bool random_seed, unsigned n_cells_wide,
		...
		std::vector<Node<3>*> nodes;
		GenerateRandomHCPNodes(nodes, n_cells_wide,n_cells_deep,n_cells_high, max_noise);
	 ...
	 */
	void GenerateRandomHCPNodes(std::vector<Node<3>*> & rNodes,
			unsigned n_nodes_width,
			unsigned n_nodes_depth,
			unsigned n_nodes_height,
			double max_noise) throw(Exception)
