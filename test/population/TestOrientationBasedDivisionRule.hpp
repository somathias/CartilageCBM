...
class TestOrientationBasedDivisionRule : public AbstractCellBasedTestSuite
{
  public:
     ...
     void TestHorizontalDirectedDivision()
    {
        ...
        TS_ASSERT_EQUALS(my_coords(1) + 0.5 * separation * sin(random_angle), your_coords(1));
        TS_ASSERT_EQUALS(my_coords(2), your_coords(2));

        TS_ASSERT_EQUALS(my_coords(0) - 0.5 * separation * cos(random_angle), my_new_coords(0));
        TS_ASSERT_EQUALS(my_coords(1) - 0.5 * separation * sin(random_angle), my_new_coords(1));
        TS_ASSERT_EQUALS(my_coords(2), my_new_coords(2));
    }
};
