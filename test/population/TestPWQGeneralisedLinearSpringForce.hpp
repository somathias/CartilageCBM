/*
 * TestPWQGeneralisedLinearSpringForce.hpp
 *
 *  Created on: Apr 30, 2019
 *      Author: Sonja Mathias
 */

#ifndef TESTPWQGENERALISEDLINEARSPRINGFORCE_HPP_
#define TESTPWQGENERALISEDLINEARSPRINGFORCE_HPP_
...
class TestPWQGeneralisedLinearSpringForce: public AbstractCellBasedTestSuite {
public:


	void TestPWQGeneralisedLinearSpringForceWithNodeBasedCellPopulation() {
		...
		// Create force
		PWQGeneralisedLinearSpringForce<3> force;
	...
	void TestPWQGeneralisedLinearSpringForceRepulsionOnly() {
    ...
    void TestPWQGeneralisedLinearSpringForceOutputParameters()
	{
		EXIT_IF_PARALLEL;
		std::string output_directory = "TestForcesOutputParameters";
		OutputFileHandler output_file_handler(output_directory, false);

		// Test with PWQGeneralisedLinearSpringForce
		PWQGeneralisedLinearSpringForce<2> linear_force;
		linear_force.SetCutOffLength(1.5);
		TS_ASSERT_EQUALS(linear_force.GetIdentifier(), "PWQGeneralisedLinearSpringForce-2-2");

		out_stream linear_force_parameter_file = output_file_handler.OpenOutputFile("PWQ_results.parameters");
		linear_force.OutputForceParameters(linear_force_parameter_file);
		linear_force_parameter_file->close();

		{
			FileFinder generated_file = output_file_handler.FindFile("PWQ_results.parameters");
			FileFinder reference_file("projects/cartilage/test/data/TestPWQGeneralisedLinearSpringForce/PWQ_results.parameters",
    ...
    void TestPWQGeneralisedLinearSpringForceArchiving() 
	{
		EXIT_IF_PARALLEL; // Beware of processes overwriting the identical archives of other processes
		OutputFileHandler handler("archive", false);
		std::string archive_filename = handler.GetOutputDirectoryFullPath() + "PWQGeneralisedLinearSpringForce.arch";

		{
			PWQGeneralisedLinearSpringForce<2> force;
...

		{
			...
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetMeinekeSpringStiffness(), 12.34, 1e-6);
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetMeinekeDivisionRestingSpringLength(), 0.856, 1e-6);
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetMeinekeSpringGrowthDuration(), 2.593, 1e-6);
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetMeinekeDivisionRestingSpringLength(), 0.856, 1e-6);
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetMeinekeSpringGrowthDuration(), 2.593, 1e-6);
			TS_ASSERT_DELTA((static_cast<PWQGeneralisedLinearSpringForce<2>*>(p_force))->GetRepulsionSpringStiffness(), 15.67, 1e-6);
...
#endif /* TESTPWQGENERALISEDLINEARSPRINGFORCE_HPP_ */
